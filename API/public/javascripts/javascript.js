function filtroFilmes(){
	...
	$.get('https://tv-v2.api-fetch.website/movies/125?sort=last%20added&order=1', function(data) {
		console.log(data);
		for(var x=0; x<50; x++){
			$('#quadros').append('<div class="col-md-4 imagem"><h2>'+data[x].title+'</h2><p><div class="grid"><figure class="effect-zoe"><a href="http://localhost:60000caneca/detalhada/'+data[x].id+'"><img src='+data[x].images.banner+'><figcaption><p class="icon-links"><a href="#"><i class="material-icons small"> shopping_cart</i></a><a href="#"><i class="material-icons small"> star</i></a></p></figcaption></a></figure><div><h1> Duração: '+data[x].runtime+' minutos</h1><p>'+data[x].year+'</p></div></div></p></div>');
		}
	});
}
